...
1. **회원가입**
    ...
    **프론트엔드 Validation 규칙** (2026-02-08 구현):
    - **이메일**: 
        - 필수 입력
        - 올바른 이메일 형식 검증 (yup email validation)
    - **비밀번호**:
        - 필수 입력
        - 최소 8자 이상
        - 영문 대문자, 소문자, 숫자를 모두 포함
        - 정규식: `/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/`
    - **비밀번호 확인**:
        - 필수 입력
        - 비밀번호 필드와 일치해야 함
    ...
    - **전화번호**:
        - 필수 입력
        - 형식: `01XXXXXXXXX` (하이픈 없이 11자리, 010/011/016/017/018/019)
        - 정규식: `/^01[0-9]{8,9}$/`
    - **역할 (Role)**:
        - 필수 선택
        - `BUYER` 또는 `SELLER` 중 하나
        - 기본값: `BUYER`
    
    **구현 기술**: react-hook-form + yup validation, HTML5 기본 validation 비활성화(noValidate)
...
2. **로그인/인증**
    ...
    **구현 기술**: react-hook-form + yup validation
